...
@Component({
  moduleId: module.id,
  selector: 'app-brand-label-replacement',
  templateUrl: 'brand-label-replacement.component.html'
})
export class BrandLabelReplacementComponent implements OnInit, OnDestroy {
...
ngOnInit() {
  ...
  this.titleService.setTitle(this.assignTaskResources.brandlablereplacetitle);
  ...
  if (String(this.PageFlag.page).toLocaleUpperCase() !== 'TASKACTION') {
    ...
    this.ParentFormGroup.addControl('REBRAND', this.REBRAND);
...
get aOrderDetailsArr(): FormArray {
  return (this.ParentFormGroup.get('REBRAND') as FormGroup).get('aOrderDetailsArr') as FormArray;
}

get bOrderDetailsArr(): FormArray {
  return (this.ParentFormGroup.get('REBRAND') as FormGroup).get('bOrderDetailsArr') as FormArray;
}

get cOrderDetailsArr(): FormArray {
  return (this.ParentFormGroup.get('REBRAND') as FormGroup).get('cOrderDetailsArr') as FormArray;
}
...
getLableOrders() {
  ...
  this.orderService.getLableOrders(editmode).subscribe(
    (data: any) => {
...
// On Order Change Bind order details again
 onOrderChange() {
  ...
  // Added by Devdan :: 20-Oct-2018
  this.getLableOrderDetails(this.TaskModel.REBRAND.orderno);
}

// GetQACheckOrderDetails(OrderId)
getLableOrderDetails(OrderId) {
  ...
  this.orderService.getLableOrderDetails(OrderId, editMode, this.taskId).subscribe(
    (data: any) => {
      if (data !== 'No data found!') {
        ...
        (this.ParentFormGroup.controls['REBRAND'] as FormGroup).setControl('aOrderDetailsArr', this.fb.array([]));
        (this.ParentFormGroup.controls['REBRAND'] as FormGroup).setControl('bOrderDetailsArr', this.fb.array([]));
        (this.ParentFormGroup.controls['REBRAND'] as FormGroup).setControl('cOrderDetailsArr', this.fb.array([]));
        ...
        this.qAOrderDetails.cOrderDetails.map((object, index) => {
         ...
         if (this.taskTypeId > 0) {
          this.TaskModel.RebrandOrderDetails.forEach(Ord => {
...
createPkgItem(object, index): FormGroup {
  ...
  const pkgsSelectedDetails = this.selectedFailedPkgsMap[object.ProductTypeId];
    ...
    if (pkgsSelectedDetails) {
      ...
      if (pkgRowDetails.length) {
        isPkgLabeledBox = pkgRowDetails[0].isPkgLabeled;
...
getPkgsInfo(orderDetails, Flag) {
  ...
  if (String(Flag).toLocaleUpperCase() === 'LABEL') {
      ...
      pkgAssignDetails.push(
        {
          ...
          oilPkgId: result.oilPkgId,
          oilPkgCode: result.oilPkgCode,
          isPkgLabeled: result.isPkgLabeled ? result.isPkgLabeled : false,
          ordProdItemId: Number(result.ordProdItemId),
          mixPkgId: result.mixPkgId ? result.mixPkgId : 0
...
// Commented by Devdan :: 29-Oct-2018 :: Unused
// ShowPopupNew() {
//   this.PageFlag.showmodal = true;
// }

estStartDate_Select() {
  if (((new Date(this.returnFormattedDate(this.defaultDate)) >
    new Date(this.REBRAND.value.estimatedstartdate)))) {
...
submitCompleteParameter(formModel) {
  ...
  setFormInEditMode() {
    this.TaskModel.REBRAND = {
      ...
      usercomment: this.TaskModel.RebrandTaskDetails.Comment,
  ...
  setSelectedLotDetails() {
    const pkgAssignDetails = [];
    let Package: any;
    if (this.TaskModel.RebrandBudPkgDetails.length > 0) {
      Package = this.TaskModel.RebrandBudPkgDetails;
    ...
    if (this.TaskModel.RebrandOilPkgDetails.length > 0) {
      Package = this.TaskModel.RebrandOilPkgDetails;
      Package.forEach(QAPkg => {
        if (QAPkg.ProductTypeId === this.selectedProductType.ProductTypeId) {
          pkgAssignDetails.push(
            {
    ...
    if (this.TaskModel.RebrandJointPkgDetails.length > 0) {
      Package = this.TaskModel.RebrandJointPkgDetails;
