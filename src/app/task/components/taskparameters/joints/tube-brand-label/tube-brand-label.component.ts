...
compQtyOnChange(index, formArrayObj) {
  ...
  if (formArrayObj.get('completedQty').valid) {
      ...
      if (this.disableCompletedQtyArr[index] !== 0) {
        ...
        this.TaskModel.AssignQtyLotDetails
        .filter(result => result.ProductTypeId === this.TaskModel.AssignQtyDetails[index].ProductTypeId)
        .map((object, index1) => {
...
assignQtyOnChange(jointsOrderPacket, maxValue) {
  ...
  getSelectedOrderDetails(OrderId) {
    ...
    this.orderService.getSelectedOrderDetails(OrderId, 'TUBE', editmode, this.taskId).subscribe(
      data => {
        if (data !== 'No data found!') {
  ...
  openLotSelection(rowData, rowIndex) {
    ...
    // const selectedStrainUnitValues = this.orderDetails.filter(result =>  result.StrainId === rowData.StrainId);
    const selectedStrainUnitValues = this.orderDetailsBS.filter(result =>  result.StrainId === rowData.StrainId);
  ...
  setSelectedLotDetails(objOrder) {
    ...
    this.orderDetailsBS_filteredData.forEach((order, index) => {
      ...
      this.TaskModel.TubeLabelingLotDetails.forEach(Lot => {
        if (Lot.StrainId === order.StrainId) {
          lotDetails.push(
            {
              ProductTypeId: '',
              LotNo: Lot.LotId,
        ...
        if (this.TaskModel.TubeLabelingMixPackageDetails.length > 0) {
          ...
          if (mixPkg.StrainId === order.StrainId) {
            mixPkgDetails.push(
              {
